...
class AIAssistantAction(Document):
    def validate(self):
        ...
        """
        previous = (
            frappe.db.get_value(self.doctype, self.name, "status") if self.name else None
        )
        if not previous:
        ...
        if previous in TERMINAL_STATUSES and self.status not in TERMINAL_STATUSES:
            frappe.throw(
                frappe._(
                    "AI Assistant Action {0} is {1} and cannot be reopened as {2}."
                ).format(self.name, previous, self.status)
            )
