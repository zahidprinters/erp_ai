...
def get_org_patterns():
	...
	"""
	out = {
		"top_intents": _group(["intent", "count(name) as n"], "intent", "n desc"),
		"failure_intents": _group(["intent", "count(name) as n"], "intent", "n desc", {"outcome": "failed"}),
		"corrected": _group(["intent", "sum(correction_count) as corrections"], "intent", "corrections desc"),
	}
	try:
...
def _live_block():
	...
	fails = [f"{r['intent']} ({r['n']})" for r in p["failure_intents"][:3] if r.get("intent")]
	if fails:
		lines.append(
			"Tasks that commonly fail here: "
			+ ", ".join(fails)
			+ " — pre-empt the usual mistake and explain the correct path."
		)
	corr = [
		f"{r['intent']} ({r['corrections']})"
		for r in p["corrected"][:3]
		if r.get("intent") and r.get("corrections")
	]
	if corr:
		lines.append(
			"Tasks where users often override the first answer: "
			+ ", ".join(corr)
...
def _pattern_block():
	...
	def _pick(field, limit, minimum=1):
		...
		return [
			f"{r['intent']} ({int(r.get(field) or 0)})"
			for r in ranked
