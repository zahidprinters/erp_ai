...
def _aggregate(dt, expr, filters=None):
	...
	if not frappe.has_permission(dt, "read"):
		return None
	try:
		rows = _fetch(dt, filters=filters or {}, fields=[f"{expr} as total"], limit_page_length=0)
	except Exception:
		return None
		...
		for phrase, dt in sorted(_DATA_WORDS.items(), key=lambda kv: -len(kv[0])):
			if re.search(r"(?<!\w)" + re.escape(phrase) + r"(?!\w)", q):
				...
				if n is not None:
					return {
						"ok": True,
						"answer": f"There are {n:,} {_plural(n, phrase)}.",
						"intent": "count",
						"target_doctype": dt,
					}
	return {"ok": False}
	...
	try:
		...
		for f in candidates:
			hits = frappe.get_list(
				doctype, filters={f: ["like", f"%{name}%"]}, limit_page_length=1, pluck="name"
			)
			if hits:
...
def _assistant_reply(prompt, session=None, model=None):
	...
	try:
		...
		_record_behavior(
			prompt=prompt, session_id=session, intent="llm_chat", outcome="failed", failure_reason=str(e)
		)
...
def ask_with_doc(doctype, name, prompt, session=None, model=None):
	...
	grounded = f"The user is looking at {doctype} '{name}'. Document data:\n{ctx}\n\nUser question: {prompt}"
...
def _handle_view_intent(prompt, mcp):
	...
	_doc_match = re.search(
		r"\b(show|view|details|dekho|dikhhao)\s+(?:of\s+)?(?:item\s+|invoice\s+|customer\s+)?([A-Z][A-Z0-9\-]{2,20})",
		prompt,
		re.I,
	)
	if _doc_match:
...
def _handle_report(report, mcp):
	...
	result = mcp.call_tool(
		"query_doctype",
		{"doctype": doctype, "filters": config.get("filters", {}), "fields": ["name"], "limit": 10000},
	)
...
def _fallback_to_llm(prompt, session, user, model):
	...
	for _iteration in range(_OPERATOR_MAX_ITERS):
		...
		if name.startswith(("create_", "update_", "delete_", "submit_")):
			...
			observation = json.dumps({"tool": name, "parameters": params, "result": result}, default=str)
		reply = ask_llm(
			system
			+ "\n\nUser: "
			+ prompt
	...
	if voice and response_text:
		...
		if is_voice_enabled():
			try:
				...
				log_error_safely(
					"erp_ai: text_to_speech failed",
					str(e),
					context={"session": result.get("session"), "user": getattr(frappe.session, "user", None)},
					retryable=e,
				)
...
def health():
	...
	return {
		...
		"voice_runtime": os.path.exists(os.path.join(_ai_home(), "whisper.cpp/build/bin/whisper-cli")),
...
def check_my_permission(doctype, action="read"):
	...
	return {
		...
		"action": action,
		"allowed": allowed,
		"category": get_doctype_category(doctype),
	}
...
def list_available_models(provider=None, api_key="", base_url=""):
	...
	provider_methods = {
		...
		"lm_studio": lambda: _list_openai_compat_models(base_url or "http://localhost:1234/v1", api_key),
		...
		"custom": lambda: _list_openai_compat_models(base_url or "http://localhost:8080/v1", api_key),
	}
	...
	try:
		...
		frappe.throw(_("Failed to list models for {0}").format(provider))
