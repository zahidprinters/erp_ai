	...
	(function () {
		...
		function getSession() {
			...
			if (!s) {
				...
				localStorage.setItem("ai_session", s);
			}
			return s;
		}
		...
		function bubble(msg, who, session) {
			...
			b.style.cssText =
				"max-width:80%;padding:8px 11px;border-radius:12px;font-size:13px;line-height:1.5;white-space:pre-wrap;word-break:break-word;box-shadow:0 1px 2px rgba(0,0,0,.12);" +
				(mine
			...
			if (!mine && session) {
				...
				const lock = function () {
					up.style.opacity = ".35";
					down.style.opacity = ".35";
					up.onclick = down.onclick = null;
				};
				up.onclick = function () {
					lock();
					frappe.call({
						method: "erp_ai.api.record_feedback",
						args: { session: session, helpful: 1 },
					});
				};
				down.onclick = function () {
					...
					frappe.call({
						method: "erp_ai.api.record_feedback",
						args: { session: session, helpful: 0 },
					});
				};
				fb.appendChild(up);
		...
		function addMsg(container, msg, who, session) {
			container.appendChild(bubble(msg, who, session));
		...
		function send(container, inp, session, doctype, name) {
			...
			const args =
				doctype && name
					? { doctype: doctype, name: name, prompt: q, session: session }
					: { prompt: q, session: session };
			const method =
				doctype && name ? "erp_ai.api.ask_with_doc" : "erp_ai.api.ask_v2_with_voice";
			frappe
				.call({ method: method, args: args })
				.then(function (r) {
					typing.remove();
					addMsg(container, r.message.response, "ai", session);
					if (r.message.audio_url) {
						var audio = new Audio(r.message.audio_url);
						audio.play().catch(function (e) {});
					}
				})
				.catch(function (e) {
					typing.remove();
					addMsg(container, "Sorry, an error: " + (e.message || "try again"), "ai");
				});
		}
		...
		function buildWidget() {
			...
			toggle.style.cssText =
				"position:fixed;right:24px;bottom:24px;width:56px;height:56px;border-radius:50%;background:#2490ef;color:#fff;display:flex;align-items:center;justify-content:center;font-size:26px;cursor:pointer;z-index:9999;box-shadow:0 3px 12px rgba(0,0,0,.4);";

			var panel = document.createElement("div");
			panel.id = "ai-chat-panel";
			panel.style.cssText =
				"position:fixed;right:24px;bottom:90px;width:380px;height:520px;background:var(--card-bg,#fff);border-radius:14px;box-shadow:0 8px 32px rgba(0,0,0,.32);display:none;flex-direction:column;z-index:9999;overflow:hidden;";
			panel.innerHTML =
			...
			function doSend() {
				send(container, inp, session, curDoc && curDoc.doctype, curDoc && curDoc.name);
			}
			panel.querySelector("#ai-chat-send").onclick = doSend;
			inp.addEventListener("keydown", function (e) {
				if (e.key === "Enter") doSend();
			});
			panel.querySelector("#ai-chat-close").onclick = function () {
				panel.style.display = "none";
			};
			panel.querySelector("#ai-clear").onclick = function () {
				container.innerHTML = "";
			};
			panel.querySelector("#ai-voice").onclick = function () {
				var btn = this;
				var enabled = btn.getAttribute("data-voice") === "1";
				btn.setAttribute("data-voice", enabled ? "0" : "1");
			...
			micBtn.onclick = function () {
				if (!window.isSecureContext) {
					addMsg(
						container,
						"Microphone needs HTTPS (or localhost). Open the site via https:// or the localhost URL.",
						"ai"
					);
					return;
				}
					...
					.then(function (stream) {
						...
						mr.onstop = function () {
							...
							fr.onloadend = function () {
								...
								frappe
									.call({
										method: "erp_ai.api.voice_to_text",
										args: { audio: b64, fmt: "webm" },
									})
									.then(function (r) {
									...
									.catch(function (e) {
										...
										addMsg(
											container,
											"Voice error: " + (e.message || "try again"),
											"ai"
										);
									});
							};
			...
			hello.textContent =
				"Ask anything — including counts (e.g. 'total stock', 'how many items', 'unpaid invoices').";
		...
		function injectFormButton() {
			...
			btn.onclick = function () {
				buildWidget();
				window.__erpAi.askWithDoc(frm.doctype, frm.docname);
			};
			toolbar.append(btn);
		}
		...
		function mountWorkspaceChat() {
			...
			mount.innerHTML =
				'<div class="ai-embedded-chat-wrapper" style="min-height:500px;"><p style="text-align:center;color:#888;padding-top:40px;">Loading AI chat...</p></div>';
		...
		function buildWorkspaceChat(w, mount) {
			...
			msgs.style.cssText =
				"flex:1;overflow-y:auto;padding:12px;background:var(--control-bg,#f6f8fa)";
			var ctrl = document.createElement("div");
			ctrl.style.cssText =
				"display:flex;border-top:1px solid var(--border-color,#e2e2e2);padding:8px;background:var(--card-bg,#fff)";
			...
			function add(who, msg, session) {
				...
				var mine = who === "user";
				b.style.cssText =
					"padding:8px 11px;border-radius:12px;font-size:13px;line-height:1.5;white-space:pre-wrap;word-break:break-word" +
					(mine
						? ";background:#2490ef;color:#fff;margin-left:auto;display:inline-block"
			...
			function doSend() {
				...
				msgs.scrollTop = msgs.scrollHeight;
				var args = curDoc
					? { doctype: curDoc.doctype, name: curDoc.name, prompt: q, session: session }
					: { prompt: q, session: session };
				...
				frappe
					...
					.then(function (r) {
						typing.remove();
						add("ai", r.message.response, session);
						if (r.message.audio_url) {
							var audio = new Audio(r.message.audio_url);
							audio.play().catch(function (e) {});
						}
			...
			micBtn.onclick = function () {
				if (!window.isSecureContext) {
					add("ai", "Microphone needs HTTPS (or localhost).");
					return;
				}
				var rec = window.__erpAiRec;
				if (rec && rec.state === "recording") {
				...
				navigator.mediaDevices
					...
					.catch(function (e) {
						add("ai", "Mic permission denied: " + e.name);
					});
			};

			add(
				"ai",
				"Hello from your AI assistant. Ask me about ERPNext, your data, or how-to steps."
			);
