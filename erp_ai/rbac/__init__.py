...
DEPARTMENT_RESTRICTIONS: Dict[str, Optional[Dict[str, List[str]]]] = {
	"Stock Manager": {
		"warehouses": ["Stores - SPI", "Work In Progress - SPI", "Finished Goods - SPI"],
		"companies": ["Sheikh Plastic Industries"],
	},
	"Stock User": {"warehouses": ["Stores - SPI"], "companies": ["Sheikh Plastic Industries"]},
	"Manufacturing User": {
		"warehouses": ["Work In Progress - SPI", "Stores - SPI"],
		"companies": ["Sheikh Plastic Industries"],
	},
...
def check_permission(user, doctype, action):
	...
	if not can(user, doctype, action):
		cat = get_doctype_category(doctype)
		roles = get_user_roles(user)
		return ("Access denied: your role does not allow %s on %s (%s). Your roles: %s.") % (
			action,
			doctype,
			cat,
			", ".join(roles),
