...
_AGGREGATE_RE = re.compile(r"^(count|sum|avg|min|max|group_concat)\(\s*(\w+)\s*\)\s+as\s+(\w+)$", re.I)
...
def _validate_query(
	doctype, fields=None, filters=None, order_by=None, or_filters=None, and_filters=None
) -> Optional[str]:
...
def _fetch(doctype: str, *args, **kwargs) -> List[Dict]:
	...
	if limit and int(limit) > _MAX_LIMIT:
		raise ValueError(
			f"Invalid query for {doctype}: limit_page_length {limit} exceeds the maximum of {_MAX_LIMIT}"
		)
	err = _validate_query(
		doctype,
	...
	kwargs["filters"] = apply_department_filters(frappe.session.user, doctype, kwargs.get("filters"))
	try:
...
def get_system_overview() -> Dict[str, Any]:
	...
	# Items by group
	item_groups = _fetch("Item Group", fields=["name"], order_by="name")
	...
	# Warehouses
	warehouses = _fetch(
		"Warehouse", fields=["name", "warehouse_name", "is_group", "company"], order_by="warehouse_name"
	)
	overview["warehouses"] = [
		{
			"name": w["name"],
	...
	overview["open_orders"]["sales"] = _count(
		"Sales Order", {"status": ["in", ["Draft", "Quotation", "On Hold", "Pending"]]}
	)
	overview["open_orders"]["purchase"] = _count(
		"Purchase Order", {"status": ["in", ["Draft", "On Hold", "Pending"]]}
	)
...
def get_suppliers(filters: Optional[Dict] = None, limit: int = 50) -> List[Dict]:
	...
	"""
	fields = ["name", "supplier_name", "supplier_group", "country", "website", "tax_id"]
...
def get_companies() -> List[Dict]:
	...
	"""
	fields = ["name", "company_name", "default_currency", "country", "tax_id", "website", "email", "phone_no"]
...
def get_open_sales_orders(filters: Optional[Dict] = None, limit: int = 20) -> List[Dict]:
	...
	so_filters = filters or {
		"status": ["in", ["Draft", "Quotation", "On Hold", "Pending", "Partially Delivered"]]
	}
	orders = _fetch(
		"Sales Order",
		filters=so_filters,
...
def get_low_stock_items(threshold: int = 10) -> List[Dict]:
	...
	low_stock = []
	for item in items:
		actual_stock = _sum("Stock Ledger Entry", {"item_code": item["item_code"]}, "actual_qty") or 0
...
def get_transaction_summary(days: int = 30) -> Dict[str, Any]:
	...
	summary = {
		...
		"sales_invoices": {"count": 0, "total_amount": 0, "outstanding": 0},
		"purchase_invoices": {"count": 0, "total_amount": 0, "outstanding": 0},
		"sales_orders": {"count": 0, "total_amount": 0},
		"purchase_orders": {"count": 0, "total_amount": 0},
	}
	...
	# Sales orders
	sales_orders = _fetch(
		"Sales Order", filters={"transaction_date": [">=", from_date]}, fields=["grand_total"]
	)
	...
	# Purchase orders
	purch_orders = _fetch(
		"Purchase Order", filters={"transaction_date": [">=", from_date]}, fields=["grand_total"]
	)
...
def get_available_tools() -> List[Dict]:
	...
	return [
		...
		},
		{
			...
			"parameters": {"item_code": "The item code to look up (required)"},
		},
		{
			"name": "search_items",
			"description": "Search items by name, code, description, or barcode",
			"parameters": {"query": "Search term (required)", "limit": "Max results (default 20)"},
		},
		{
			"name": "get_customers",
			"description": "Get list of all customers with details",
			"parameters": {"filters": "Optional filters", "limit": "Max customers to return (default 50)"},
		},
		{
			"name": "get_customer_details",
			"description": "Get detailed information about a specific customer",
			"parameters": {"customer_code": "Customer document name (required)"},
		},
		{
			"name": "get_suppliers",
			"description": "Get list of all suppliers with details",
			"parameters": {"filters": "Optional filters", "limit": "Max suppliers to return (default 50)"},
		},
		{
			"name": "get_supplier_details",
			"description": "Get detailed information about a specific supplier",
			"parameters": {"supplier_code": "Supplier document name (required)"},
		},
		{"name": "get_companies", "description": "Get all companies in the ERP system", "parameters": {}},
		{
			"name": "get_company_details",
			"description": "Get detailed information about a specific company",
			"parameters": {"company_name": "Company document name (required)"},
		},
		{
			"name": "get_warehouses",
			"description": "Get all warehouses or filter by group",
			"parameters": {"filters": "Optional filters (e.g., company or is_group)"},
		},
		{
		...
		},
		{
			...
			"parameters": {"threshold": "Minimum quantity to consider low stock (default 10)"},
		},
		{
			"name": "get_transaction_summary",
			"description": "Get transaction summary for recent period",
			"parameters": {"days": "Number of days to look back (default 30)"},
		},
		{
