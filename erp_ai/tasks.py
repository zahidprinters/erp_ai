...
def refresh_behavior_patterns() -> dict:
    ...
    for row in patterns.get("top_intents", []):
        if row.get("intent"):
            merged.setdefault(
                row["intent"], {"use": 0, "fail": 0, "corr": 0})["use"] = int(row["n"])
    for row in patterns.get("failure_intents", []):
        if row.get("intent"):
            merged.setdefault(
                row["intent"], {"use": 0, "fail": 0, "corr": 0})["fail"] = int(row["n"])
    for row in patterns.get("corrected", []):
        if row.get("intent"):
            merged.setdefault(
                row["intent"], {"use": 0, "fail": 0, "corr": 0})["corr"] = int(
                    row.get("corrections") or 0)
    samples = {}
    for row in patterns.get("recent_failures", []):
        if (row.get("intent") and row.get("failure_reason")
                and row["intent"] not in samples):
    ...
    stale = [name for name in frappe.get_all(PATTERN_DOCTYPE, pluck="name")
             if name not in merged]
    for name in stale:
        try:
            frappe.delete_doc(PATTERN_DOCTYPE, name,
    ...
    if merged or stale:
        frappe.logger("erp_ai").info(
            "erp_ai: refresh_behavior_patterns rows=%s stale=%s"
            % (len(merged), len(stale)))
...
def draft_help_for_repeated_failures(threshold: int = FAIL_THRESHOLD) -> dict:
    ...
    try:
        from erp_ai.erp_ai.doctype.ai_user_behavior import ai_user_behavior as behavior
        candidates = [r for r in behavior.get_org_patterns().get("failure_intents", [])
    ...
    for row in candidates[:10]:  # bounded: a bad night cannot spam drafts
        ...
        try:
            ...
            if frappe.get_all(ARTICLE_DOCTYPE,
            ...
            sample = frappe.get_all(
                ...
                fields=["failure_reason"], order_by="creation desc",
                limit_page_length=1)
            reason = str((sample or [{}])[0].get("failure_reason")
                         or "no recorded failure reason")[:400]
            frappe.get_doc({
    ...
    if drafted or skipped:
        frappe.logger("erp_ai").info(
            "erp_ai: draft_help_for_repeated_failures drafted=%s skipped=%s"
            % (drafted, skipped))
    return {"drafted": drafted, "skipped_existing": skipped,
