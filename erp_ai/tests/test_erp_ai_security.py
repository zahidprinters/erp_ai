...
class TestDocLevelPermissionGuard(unittest.TestCase):
    ...
    def test_workspace_tool_allows_system_manager(self):
        mcp = FrappeMCP()
        with mock.patch("frappe.get_roles", return_value=["System Manager"]), \
    ...
    def test_print_document_rejects_unknown_format(self):
        ...
        with mock.patch("frappe.has_permission", side_effect=[True, True]), \
...
class TestWritePayloadSanitizer(unittest.TestCase):
    ...
    def test_create_document_drops_every_blocked_field(self):
        ...
        mcp = FrappeMCP()
        with mock.patch("frappe.has_permission", side_effect=[True]), \
        ...
        for field in self.BLOCKED:
            ...
            self.assertNotIn(field, captured,
                             "sanitizer let a framework-managed field through: %s" % field)
...
class TestQueryPermissionScoping(_SkipIfNoDB, unittest.TestCase):
    ...
    def _create_fixtures(self):
        frappe.get_doc({
            "doctype": "User", "email": self.USER, "first_name": "AI6 Restricted",
            "send_welcome_email": 0, "roles": [{"role": self.ROLE}],
        ...
        for code in (self.ITEM_A, self.ITEM_B):
            ...
            frappe.get_doc({
                "doctype": "Bin", "item_code": code, "warehouse": self.warehouse,
                "actual_qty": 5,
            }).insert(ignore_permissions=True)
        # Scope the user to ITEM_A only; the Bin rows follow through item_code.
        frappe.get_doc({
            "doctype": "User Permission", "user": self.USER, "allow": "Item",
            "for_value": self.ITEM_A, "apply_to_all_doctypes": 1,
        }).insert(ignore_permissions=True)
    ...
    def test_mcp_query_tool_hides_documents_outside_user_scope(self):
        ...
        def _names():
            return {r["item_code"] for r in mcp.tool_query_doctype(
                "Item", filters=filters, fields=fields, limit=50)["records"]}

        self.assertEqual(_names(), {self.ITEM_A, self.ITEM_B})
        self.assertEqual(self._as_restricted(_names), {self.ITEM_A},
    ...
    def test_mcp_search_tool_hides_documents_outside_user_scope(self):
        ...
        scoped = self._as_restricted(lambda: {
            r["name"] for r in mcp.tool_search_documents("AI6-", doctype="Item", limit=50)["results"]})
        self.assertEqual(scoped, {self.ITEM_A},
                         "tool_search_documents returned an Item outside the user's scope")
    ...
    def test_count_intent_is_permission_scoped(self):
        ...
        self.assertEqual(self._as_restricted(lambda: api_mod._count("Item", filters)), 1,
    ...
    def test_bin_sum_intent_is_permission_scoped(self):
        ...
        self.assertLess(readable, raw,
                        "fixture invalid: the user's scope did not filter Bin rows")
        ...
        def _dept_scoped_sum():
            rows = frappe.get_list("Bin", fields=["actual_qty", "warehouse"],
        ...
        shown = frappe.utils.flt(
            answer["answer"].split("units")[0].split(":")[-1].strip().replace(",", ""))
        self.assertEqual(shown, dept_scoped,
                         "bin_sum ignored the department warehouse scope")
        self.assertLessEqual(shown, readable,
                             "bin_sum answered with the permission-blind total")
    ...
    def test_resolve_item_hides_documents_outside_user_scope(self):
        ...
        self.assertEqual(self._as_restricted(lambda: resolve_item(self.ITEM_B)), [],
    ...
    def test_mcp_get_document_denies_document_outside_user_scope(self):
        ...
        self.assertIn("error", denied,
                      "tool_get_document returned a document outside the user's scope")
    ...
    def test_mcp_print_document_denies_document_outside_user_scope(self):
        ...
        self.assertIn("error", denied,
                      "tool_print_document exposed a document outside the user's scope")
    ...
    def test_mcp_update_document_denies_document_outside_user_scope(self):
        ...
        denied = self._as_restricted(lambda: mcp.tool_update_document(
            "Item", self.ITEM_B, {"description": "scope escape"}))
        self.assertIn("error", denied,
                      "tool_update_document modified a document outside the user's scope")
    ...
    def test_mcp_update_document_respects_document_level_scope(self):
        ...
        frappe.get_doc({
            "doctype": "User", "email": write_user, "first_name": "AI6 ItemMgr",
            "send_welcome_email": 0, "roles": [{"role": "Item Manager"}],
        }).insert(ignore_permissions=True)
        self.addCleanup(self._delete_scoped_user, write_user)
        frappe.get_doc({
            "doctype": "User Permission", "user": write_user, "allow": "Item",
            "for_value": self.ITEM_A, "apply_to_all_doctypes": 1,
        }).insert(ignore_permissions=True)
        frappe.clear_cache()
        frappe.cache.delete_keys("*user_permissions*")
        ...
        self.assertIn("error", denied,
                      "tool_update_document modified a document outside the user's scope")
        self.assertIn(self.ITEM_B, denied["error"],
                      "denied by the doctype-level check, not the document-level one")
        self.assertEqual(frappe.db.get_value("Item", self.ITEM_B, "description"), before)

        allowed = _write(self.ITEM_A, "AI6 in-scope update")
        self.assertTrue(allowed.get("success"), allowed)
        self.assertEqual(frappe.db.get_value("Item", self.ITEM_A, "description"),
                         "AI6 in-scope update")
    ...
    def test_department_scope_blocks_out_of_scope_aggregate(self):
        ...
        if not frappe.db.exists("Bin", {"item_code": self.ITEM_A, "warehouse": outside_wh}):
            frappe.get_doc({
                "doctype": "Bin", "item_code": self.ITEM_A,
                "warehouse": outside_wh, "actual_qty": 9,
            }).insert(ignore_permissions=True)
            frappe.db.commit()
        ...
        self.assertLess(in_scope, raw,
                        "fixture invalid: no Bin exists outside the restricted warehouses")

        # The aggregate path must apply exactly the department filters.
        scoped = self._as_restricted(lambda: erp_tools._count("Bin"))
        self.assertEqual(scoped, in_scope,
                         "aggregate did not apply the department warehouse scope")
        self.assertLess(scoped, raw,
                        "department-restricted user aggregated Bins outside their warehouse scope")
    ...
    def test_department_scope_blocks_adversarial_company_filter(self):
        ...
        user = self._ensure_dept_user()
        try:
            scoped = self._as_user(
                user, lambda: erp_tools._count("Warehouse", {"company": other_company}))
            self.assertEqual(scoped, 0,
                             "adversarial company filter escaped the department scope")
        finally:
            self._drop_dept_user()
    ...
    def _ensure_dept_user(self):
        if not frappe.db.exists("User", self.DEPT_USER):
            frappe.get_doc({
                "doctype": "User", "email": self.DEPT_USER,
                "first_name": "AI7 Dept Scope", "send_welcome_email": 0,
    ...
    def test_operator_tool_create_requires_create_permission(self):
        ...
        try:
            res = self._as_user("Guest", lambda: execute_erp_tool("create_item", {
                "item_code": self.GUEST_ITEM, "item_name": "AI6 Guest Item",
                "item_group": "Products"}))
        ...
        self.assertIsNone(res.get("result"),
                          "a user without create permission created an Item: %s" % res)
...
class TestLiveMetadataRequiredFields(_SkipIfNoDB, unittest.TestCase):
    ...
    def test_item_with_basics_passes_validation(self):
        ...
        err = _validate_required_fields("Item", {
...
class TestStockIssueNL(_SkipIfNoDB, unittest.TestCase):
    ...
    def test_issue_nl_supplies_from_warehouse_and_creates_draft(self):
        ...
        mcp = FrappeMCP()
        res = handle_issue_nl(
            "issue 5 %s from %s to John" % (self.ITEM, self.warehouse), mcp)
...
class TestMCPRpcAuthorization(_SkipIfNoDB, unittest.TestCase):
    ...
    def test_write_requires_manager_role(self):
        ...
        with mock.patch("frappe.get_roles", return_value=["Stock User"]):
            with self.assertRaises(frappe.PermissionError):
                mcp_rpc.mcp_call_tool("create_document",
                                      {"doctype": "Item", "data": {"item_code": "X"}})
    ...
    def test_manager_write_passes_the_rpc_gate(self):
        ...
        with mock.patch("frappe.get_roles", return_value=["Stock Manager"]):
            res = mcp_rpc.mcp_call_tool("create_document",
                                        {"doctype": "User", "data": {}})
    ...
    def test_pii_doctypes_are_not_available(self):
        for doctype in ("Employee", "Leave Application", "Attendance",
                        "Payroll Entry", "Salary Structure", "Expense Claim"):
...
class TestChatRouteSmoke(_SkipIfNoDB, unittest.TestCase):
    ...
    def test_ask_data_answer_short_circuits_llm(self):
        with mock.patch("erp_ai.api._data_answer",
                        return_value={"ok": True, "answer": "We have 3 items."}), \
    ...
    def test_ask_v2_with_voice_returns_response_and_audio(self):
        with mock.patch("erp_ai.api._data_answer", return_value={"ok": False}), \
                ...
                mock.patch("erp_ai.api.text_to_speech",
                           return_value={"url": "/files/x.wav"}) as tts:
    ...
    def test_ask_with_doc_grounds_on_document(self):
        ...
        with mock.patch("erp_ai.api._resolve", return_value=doc), \
            ...
            res = api_mod.ask_with_doc("Item", "SMOKE-ITEM", "what is this?",
                                       session="smoke-doc")
...
class TestRouteWhitelistContract(_SkipIfNoDB, unittest.TestCase):
    ...
    def test_every_decorated_route_is_registered_with_frappe(self):
        ...
        for name in routes:
            ...
            self.assertIn(fn, frappe.whitelisted,
                          "api.%s is decorated but not registered" % name)
    ...
    def test_routes_are_callable_by_keyword(self):
        ...
        for name in self._decorated_route_names():
            ...
            positional_only = [p.name for p in sig.parameters.values()
                               if p.kind is p.POSITIONAL_ONLY]
            self.assertEqual(positional_only, [],
                             "route api.%s has positional-only args %s"
                             % (name, positional_only))
            # Every parameter must be satisfiable from form_dict keyed by name.
            try:
                sig.bind(**{n: None for n, p in sig.parameters.items()
    ...
    def test_all_named_p0_routes_exist(self):
        ...
        for name in ("ask", "ask_v2", "ask_v2_with_voice", "ask_with_doc",
                     "run_evaluation", "workflow_shipment_receipt",
                     "workflow_stock_issue", "ocr_extract_text",
                     "list_available_models"):
            self.assertTrue(callable(getattr(api_mod, name, None)),
                            "api.%s is missing" % name)
    ...
    def test_mcp_rpc_surface_is_whitelisted(self):
        ...
        for name in ("mcp_call_tool", "mcp_list_tools"):
            ...
            self.assertIn(getattr(fn, "__func__", fn), frappe.whitelisted,
                          "mcp.%s is not exposed to RPC" % name)
...
class TestQueryValidationContract(_SkipIfNoDB, unittest.TestCase):
    ...
    def test_valid_order_by_with_direction_is_accepted(self):
        ...
        _fetch("Item", fields=["item_code", "modified"], order_by="modified desc",
               limit_page_length=1)
    ...
    def test_oversized_limit_is_rejected_not_clamped(self):
        ...
        self.assertIsInstance(
            _fetch("Item", fields=["item_code"], limit_page_length=_MAX_LIMIT), list)
    ...
    def test_search_items_matches_on_name_code_and_description(self):
        ...
        if not frappe.db.exists("Item", code):
            frappe.get_doc({
                "doctype": "Item", "item_code": code, "item_name": "Zeta Probe Widget",
                "item_group": "Products", "stock_uom": "Nos", "is_stock_item": 0,
            }).insert(ignore_permissions=True)
            frappe.db.commit()
            self.addCleanup(lambda: frappe.delete_doc(
                "Item", code, ignore_permissions=True, force=True))
    ...
    def test_validation_is_skipped_when_doctype_metadata_is_absent(self):
        ...
        self.assertIsNone(_validate_query("Not A Real Doctype",
                                          fields=["whatever"], filters={"x": "y"}))
...
class TestOperatorToolAuthorization(_SkipIfNoDB, unittest.TestCase):
    ...
    def test_rejected_create_is_reported_as_a_failure(self):
        ...
        with mock.patch.dict("erp_ai.erp_tools.ERP_TOOLS",
                             {"create_probe": lambda **kw: None}):
    ...
    def test_arbitrary_admin_tools_are_not_reachable(self):
        ...
        for name in ("grant_role", "frappe.client.insert", "get_doc",
                     "delete_document", "submit_document", "__import__",
                     "system_console"):
            self.assertFalse(execute_erp_tool(name, {})["success"],
                             "%s must not be dispatchable" % name)
    ...
    def _ensure_restricted_user(self):
        if not frappe.db.exists("User", self.RESTRICTED_USER):
            frappe.get_doc({
                "doctype": "User", "email": self.RESTRICTED_USER,
                "first_name": "AI6 Operator Restricted", "send_welcome_email": 0,
    ...
    def _drop_restricted_user(self):
        ...
        if frappe.db.exists("User", self.RESTRICTED_USER):
            frappe.delete_doc("User", self.RESTRICTED_USER,
    ...
    def test_restricted_user_cannot_create_customer_via_operator_tool(self):
        ...
        self.assertFalse(result.get("success"),
                         "operator tool reported success for a denied create")
...
class TestErrorLoggingNeverMasksFailures(_SkipIfNoDB, unittest.TestCase):
    ...
    def test_execute_erp_tool_reports_the_real_error_on_failure(self):
        ...
        with mock.patch.dict("erp_ai.erp_tools.ERP_TOOLS",
                             {"probe_tool": mock.Mock(side_effect=boom)}):
...
class TestOperatorCreatePayloadContract(_SkipIfNoDB, unittest.TestCase):
    ...
    def _assert_contract(self, payload):
        ...
        for key, value in payload.items():
            ...
            if child_field and key == child_field:
                child_allowed = (set(child.get("required", []))
                                 | set(child.get("optional", [])))
                for row in value:
                    self.assertEqual(
                        set(row) - child_allowed, set(),
                        "%s.%s[] carries dropped keys %s"
                        % (doctype, key, set(row) - child_allowed))
                continue
            self.assertIn(key, allowed,
                          "%s payload key %r is dropped by the creator" % (doctype, key))
        return payload
    ...
    def test_item_payload_is_inside_the_schema_contract(self):
        ...
        payload = self._capture_payload(
            create_item, item_code="AI6-CONTRACT-ITEM", item_name="AI6 Contract Item",
            item_group="Products", description="from the tool", is_stock_item=0,
            standard_rate=11, valuation_rate=7)
    ...
    def test_customer_payload_is_inside_the_schema_contract(self):
        ...
        payload = self._capture_payload(
            create_customer, customer_name="AI6 Contract Customer",
            phone_no="03001234567", email_id="contract@example.com")
        self._assert_contract(payload)
    ...
    def test_supplier_payload_is_inside_the_schema_contract(self):
        ...
        payload = self._capture_payload(
            create_supplier, supplier_name="AI6 Contract Supplier",
            phone_no="03001234568", email_id="supplier@example.com")
        self._assert_contract(payload)
    ...
    def test_sales_order_payload_is_inside_the_schema_contract(self):
        ...
        payload = self._capture_payload(
            create_sales_order, customer="AI6 Contract Customer",
            items=[{"item_code": "AI6-CONTRACT-ITEM", "qty": 2, "rate": 5}],
            delivery_date="2026-01-01", company=company, warehouse=warehouse,
            currency="PKR", comments="please expedite")
        self._assert_contract(payload)
