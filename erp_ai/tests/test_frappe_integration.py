...
class TestIdempotencyUnique(FrappeTestCase):
	...
	def test_claim_new_then_pending_is_duplicate(self):
		key = _uniq("idem")
		r1 = claim_idempotency(key, "sess", "Administrator", "create", "Item", {"item_name": "X"})
		self.assertEqual(r1["status"], "new")
		r2 = claim_idempotency(key, "sess", "Administrator", "create", "Item", {"item_name": "X"})
		self.assertEqual(r2["status"], "duplicate")
		self.assertEqual(r2["action_id"], r1["action_id"])

	def test_claim_forbidden_for_other_user(self):
		key = _uniq("idem_forbid")
		claim_idempotency(key, "sess", "Administrator", "create", "Item", {"item_name": "X"})
		r2 = claim_idempotency(key, "sess", "guest@example.com", "create", "Item", {"item_name": "X"})
	...
	def test_claim_pending_blocks_new_claim(self):
		...
		r1 = claim_idempotency(key, "s1", "Administrator", "create", "Item", {"item_name": "X"})
		self.assertEqual(r1["status"], "new")
		r2 = claim_idempotency(key, "s2", "Administrator", "create", "Item", {"item_name": "Y"})
		self.assertEqual(r2["status"], "duplicate")
		self.assertEqual(r2["action_id"], r1["action_id"])
	...
	def test_completed_key_blocks_fresh_claim(self):
		...
		r1 = claim_idempotency(key, "s1", "Administrator", "create", "Item", {"item_name": "Z"})
		self.assertEqual(r1["status"], "new")
		frappe.db.set_value("AI Assistant Action", r1["action_id"], "status", "completed")
		frappe.db.commit()
		r2 = claim_idempotency(key, "s2", "Administrator", "create", "Item", {"item_name": "different"})
		self.assertEqual(r2["status"], "duplicate")
		self.assertEqual(r2["action_id"], r1["action_id"])
...
class TestAuditConfirmationSecurity(FrappeTestCase):
	...
	def _make_item_draft(self):
		...
		if ig:
			data["item_group"] = ig
		r = create_draft(
			session=session, action="create", target_doctype="Item", draft_data=data, user="Administrator"
		)
		return session, r
	...
	def test_confirm_rejects_changed_preview(self):
		session, r = self._make_customer_draft()
		frappe.db.set_value(
			"AI Assistant Action", r["name"], "draft_data", json.dumps({"customer_name": "MUTATED"})
		)
		frappe.db.commit()
		res = confirm_draft(session=session, user="Administrator")
		self.assertTrue(isinstance(res, dict))
...
class TestShipmentPreviewConfirmCancel(FrappeTestCase):
	...
	def test_workflow_receipt_cancel_rejects_another_user(self):
		res = api_mod.workflow_shipment_receipt({"text": "received 1 x TST-CANCEL-ITEM from Ship Test Ltd"})
		draft_id = res.get("draft_id")
...
class TestLiveDocTypeMetadata(FrappeTestCase):
	...
	def test_all_schema_doctypes_exist_with_required_fields(self):
		...
		self.assertEqual(
			missing_fields, [], "Required fields missing from live metadata: %s" % missing_fields
		)
...
class TestGenericDepartmentExecution(FrappeTestCase):
	...
	def _confirm(self, doctype, data):
		session = _uniq("gen")
		create_draft(
			session=session, action="create", target_doctype=doctype, draft_data=data, user="Administrator"
		)
	...
	def test_rollback_on_mid_execution_failure(self):
		...
		create_draft(
			...
			action="create",
			target_doctype="Item",
			draft_data={"item_name": session, "item_group": "Raw Material", "stock_uom": "Nos"},
			user="Administrator",
		)
		...
		actions = frappe.get_all(
			"AI Assistant Action", filters={"session_id": session}, fields=["status", "failure_reason"]
		)
