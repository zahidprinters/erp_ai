...
def test_validate_file_accepts_valid_png():
	err = validate_file("scan.png", 100, mime_type="image/png", content_head=b"\x89PNG\r\n\x1a\n\x00\x00")
	assert err is None


def test_validate_file_rejects_spoofed_extension():
	# A shell script renamed to .pdf must fail signature sniffing
	err = validate_file("evil.pdf", 100, mime_type="application/pdf", content_head=b"#!/bin/sh\nrm -rf /\x00")
...
def _stub_frappe(monkeypatch):
	...
	monkeypatch.setitem(
		sys.modules, "frappe", types.SimpleNamespace(generate_hash=lambda length=8: "deadbeef")
	)
...
# --- Phase 2.2: commit-message conventional subject check ---
def _load_commit_msg_checker():
	"""Import .github/scripts/check_commit_msg.py (not a package)."""
	import importlib.util
	import pathlib

	path = pathlib.Path(__file__).resolve().parents[2] / ".github" / "scripts" / "check_commit_msg.py"
...
def test_commit_msg_accepts_conventional_subjects():
	check = _load_commit_msg_checker().check
	assert check("feat: add voice budget") == []
	assert check("fix(api): decode audio payload") == []
	assert check("ci: ensure erp_ai is in apps.txt") == []
	assert check("docs(roadmap): mark phase 2.1 done") == []


def test_commit_msg_rejects_non_conventional_subjects():
	check = _load_commit_msg_checker().check
	assert check("updated stuff")  # no type prefix
	assert check("wip")  # no type prefix
	assert check("feat add voice budget")  # missing colon
	assert check("nosec: whatever")  # unknown type


def test_commit_msg_rejects_overlong_and_empty_subjects():
	check = _load_commit_msg_checker().check
	assert check("feat: " + "x" * 200)  # subject too long
	assert check("")  # empty message
	assert check("# comment only")  # nothing but comments


def test_commit_msg_exempts_merges_and_fixups():
	check = _load_commit_msg_checker().check
	assert check("Merge branch 'develop' into main") == []
	assert check("fixup! feat: add voice budget") == []


def test_idempotency_key_deterministic():
	a = generate_idempotency_key("sess-1", "create", "Sales Invoice", {"customer": "ABC"})
	b = generate_idempotency_key("sess-1", "create", "Sales Invoice", {"customer": "ABC"})
	assert a == b


def test_idempotency_key_differs_on_data():
	a = generate_idempotency_key("sess-1", "create", "Sales Invoice", {"customer": "ABC"})
	b = generate_idempotency_key("sess-1", "create", "Sales Invoice", {"customer": "XYZ"})
	assert a != b
...
def test_default_site_configuration_is_valid_and_local(monkeypatch):
	...
	assert (
		llm.validate_deployment_settings(
			_Settings(llm_provider="ollama|Ollama (Local - Free)", default_llm_model="qwen2.5:1.5b")
		)
		is None
	)
...
def test_ask_llm_uses_custom_endpoint_and_legacy_key(monkeypatch):
	settings = _Settings(
		llm_provider="custom",
		custom_api_base_url="http://10.0.0.5:8000/v1/chat/completions",
...
def test_validate_deployment_settings_blocks_release_blockers(monkeypatch):
	...
	assert (
		llm.validate_deployment_settings(
			_Settings(llm_provider="ollama", timeout_seconds=120, max_tokens=2048)
		)
		is None
	)
