...
def handle_create_item(prompt: str, mcp) -> str:
    ...
    return "✅ Item created: %s (%s)\n  Group: %s | Price: %s | UOM: %s" % (
        item_name, data["item_code"], data.get("item_group", "-"), data.get("standard_rate", 0), data.get("stock_uom", "-"))
...
def handle_count_query(prompt: str, mcp) -> str:
    ...
    for keyword, (dt, filters) in count_map.items():
        if keyword in pl:
            result = mcp.call_tool("query_doctype", {"doctype": dt, "filters": filters, "fields": ["name"], "limit": 10000})
