...
PROVIDERS = {
    ...
    "OpenRouter": {"base_url": "https://openrouter.ai/api/v1/chat/completions", "model_format": "provider/model"},
    ...
    "Anthropic": {"base_url": "https://api.anthropic.com/v1/messages", "model_format": "name", "type": "anthropic"},
    "OpenAI": {"base_url": "https://api.openai.com/v1/chat/completions", "model_format": "name"},
    "Google Gemini": {"base_url": "https://generativelanguage.googleapis.com/v1beta/models", "model_format": "models/name", "type": "google"},
...
def configured_model(settings):
    ...
    """
    return _configured(settings, "default_llm_model") or stored_model_name(
        _configured(settings, "llm_model")
    )
...
def validate_deployment_settings(settings):
    ...
    if PROVIDERS[provider]["base_url"].startswith("https:") and provider != "Custom API":
        if not _configured(settings, "api_key", "llm_api_key"):
            return (
                "{} requires an API key. Set API Key in AI Settings before using "
                "the assistant.".format(provider)
            )
    ...
    if provider == "Custom API":
        ...
        if not configured_model(settings):
            return (
                "Custom API requires a model name: no default model can be derived "
                "for an arbitrary endpoint."
            )

    for fieldname, label in (("timeout_seconds", "Timeout (seconds)"),
...
def get_provider_config(provider=None):
    ...
    if provider not in PROVIDERS:
        frappe.throw(
            f"Unknown LLM provider: {provider}. Set LLM Provider in AI Settings."
        )
...
def _ask_chat_completions(prompt, provider, model, api_key, temperature, max_tokens, timeout, base_url, config):
