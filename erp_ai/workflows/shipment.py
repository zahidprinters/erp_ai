...
def handle_shipment_nl(text: str, mcp) -> Dict[str, Any]:
	...
	supplier = None
	m = re.search(
		r"(?:from|supplier|vendor)\s+([A-Za-z0-9 .&-]{2,40}?)(?:\s+(?:at|in|to|vehicle|arriv)|,|\.|$)",
		text,
		re.I,
	)
	if m:
	...
	for m in re.finditer(
		r"([0-9][0-9,.]*)\s*(?:x|×)?\s*(?:pcs|pieces|nos|units|kg|boxes|sets)?\s+([A-Za-z][A-Za-z0-9 -]{2,40}?)(?=\s+(?:arriv|from|to|in|at|,|\.)|,|\.|$)",
		text,
		re.I,
	):
	...
	if not items:
		for m in re.finditer(
			r"\b((?:pump\s+)?springs?|bolts?|nuts?|bearings?|gaskets?|seals?|valves?|pumps?)\b", text, re.I
		):
...
def create_shipment_receipt(data: Dict[str, Any], mcp) -> Dict[str, Any]:
			...
			created = mcp.call_tool(
				"create_document",
				{
					...
					"data": {
						...
						"item_group": it.get("item_group", "Raw Material"),
						"stock_uom": it.get("uom", "Nos"),
						"is_stock_item": 1,
						"standard_rate": it.get("rate", 0),
					},
				},
			)
	...
	remarks = " | ".join(
		filter(
			None,
			[
				"Vehicle: %s" % data.get("vehicle_no") if data.get("vehicle_no") else None,
				data.get("remarks", ""),
			],
		)
	)
