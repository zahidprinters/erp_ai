...
def handle_issue_nl(text: str, mcp) -> Dict[str, Any]:
    ...
    m = re.search(r"(?:issue|transfer|jari)[^a-z]*([0-9,.]+)\s*(?:pcs|nos|units|kg)?\s*([A-Za-z][A-Za-z0-9 -]{2,40}?)(?:\s+to\s+|\s+from\s+|,|\.|$)", text, re.I)
    if m and m.group(2):
    ...
    warehouse = None
    m = re.search(r"(?:from|se)\s+([A-Za-z0-9 -]{2,40}?)(?:\s+warehouse|\s+store)?(?:\s+to\s+|,|\.|$)", text, re.I)
    if m:
    ...
    data = {"item_code": item_code, "qty": qty, "issue_type": "Material Issue",
            "issued_to": person, "from_warehouse": warehouse, "remarks": text[:200]}
