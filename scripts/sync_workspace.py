...
def execute():
    spec_path = pathlib.Path(__file__).resolve().parent.parent / "erp_ai" / "erp_ai" / "workspace" / "ai_assistant_hub" / "ai_assistant_hub.json"
    ...
    if frappe.db.exists("Workspace", "AI Assistant Hub"):
        ...
        ws.shortcuts = [
            frappe.new_doc("Workspace Shortcut", d)
            for d in spec.get("shortcuts", [])
        ]
        ws.save()
        frappe.db.commit()
