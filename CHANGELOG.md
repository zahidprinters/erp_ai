...
- **Phase 2.2 / 3.2 — pre-commit enforcement**: hooks now install for `pre-commit`, `pre-push` and `commit-msg` stages (a `--no-verify` commit is still caught at push); whitespace/EOF checks run repo-wide (not just `erp_ai.*`); new `check-added-large-files`; new `.github/scripts/check_commit_msg.py` commit-msg hook enforcing `<type>[(scope)]: subject`; ruff hook rev and CI's `ruff==` pin aligned to 0.16.8 (the stale 0.8.1 hook could not even parse `pyproject.toml` because `UP045` did not exist yet); existing formatting drift lint-cleaned (trailing whitespace/EOF, and prettier reformatted the three JS files); CI now runs the same hooks (`SKIP=prettier,eslint`).
- **Phase 3.1 — flow-level e2e tests**: new `erp_ai/tests/test_flows_e2e.py` asserting the user-visible contract — (a) guided conversation from a missing required field through questions, the ready preview and confirmation (document created, audit action completed, guided session cleared), and (b) draft → confirm (document created + outcome recorded on the action, with a repeat confirm reported as already handled and no duplicate document). Runs in the `frappe-tests` CI job.
